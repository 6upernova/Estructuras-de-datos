...
public class HashDictionary<K,V> implements Dictionary<K,V> {
	...
	eliminadas. Recuerde que está agregando un método a la clase Diccionario<K,V>, por lo tanto tiene total
	acceso a su estructura
	*/
	public Iterable<Entry<K,V>> eliminarTodas(K c, V v) throws InvalidKeyException {
		checkKey(c);
		PositionList<Entry<K,V>> toret = new ListDE<Entry<K,V>>();
		for (int i = 0; i < N; i++ ) 
			for( Position<Entrada<K,V>> p : buckets[i].positions()) 
